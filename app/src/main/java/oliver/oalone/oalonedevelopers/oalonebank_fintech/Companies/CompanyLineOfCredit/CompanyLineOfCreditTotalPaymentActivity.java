...
public class CompanyLineOfCreditTotalPaymentActivity extends AppCompatActivity {
    ...
    String currentUserID,credit_line_pen_total,credit_line_pen_used,credit_line_pen_available,basic_account_pen,basic_account_usd,currency_rate_sell,currency_rate_buy,credit_line_image,postKey,
            ...
            rate_accumulated,transactions_number,number_of_quotes,ammount_df;
    double credit_line_total_pen_double,credit_line_used_pen_double,credit_line_available_pen_double,currency_sell,usd_amount,amount_double,basic_account_pen_db,basic_account_usd_db,quote_amount_db,
            ...
            total_interest_amount_accumulated_db,current_interest_amount_db,rate_accumulated_db,transactions_number_db,number_of_quotes_db,monthly_interest_db;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_company_line_of_credit_total_payment);
                                ...
                                imageResourses.addValueEventListener(new ValueEventListener() {
                                    ...
                                    public void onDataChange(DataSnapshot dataSnapshot) {
                                        if (dataSnapshot.exists())
                                        {
                                            ...
                                            userRef.child(currentUserID).child("Company Line Of Credit Bills").child(postKey).addListenerForSingleValueEvent(new ValueEventListener() {
                                                @Override
                                                public void onDataChange(DataSnapshot dataSnapshot) {
                                                    ...
                                                    txtQuoteAmount.setText("S/"+quote_amount);
                                                    amount_double = Double.parseDouble(quote_amount);

                                                    rdPenAccount.setChecked(true);

                                                    txtDesgravamenInsurrance.setText("Seguro de desgravamen: S/"+desgravamen_insurrance);
                                                    txtAmountToPay.setText("Monto a pagar: S/"+total_amount_to_pay_st+" PEN");
                                                    txtQuoteAmountToPay.setText("Deuda a pagar: S/"+quote_amount);
        ...
        rdUsdAccount.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                usd_amount = total_amount_to_pay/currency_sell;
        ...
        btnPayNow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (rdPenAccount.isChecked()) {
                    if (basic_account_pen_db < total_amount_to_pay) {
                ...
                if (rdUsdAccount.isChecked()) {
                    if (basic_account_usd_db < usd_amount) {
                        Snackbar.make(findViewById(android.R.id.content), "No cuentas con dinero suficiente para realizar esta operación", Snackbar.LENGTH_LONG).show();
                        return;
                    }
                    if (basic_account_usd_db < usd_amount) {
    ...
    private void registerOperation() {
        ...
        myOperationsRef.child(currentUserID+operationRandomName).updateChildren(myOperationMap).addOnCompleteListener(new OnCompleteListener() {
            @Override
            public void onComplete(@NonNull Task task) {
                Intent intent = new Intent(CompanyLineOfCreditTotalPaymentActivity.this, CompanyLineOfCreditPayActivity.class);
                loadingBar.dismiss();
                Toasty.success(CompanyLineOfCreditTotalPaymentActivity.this, "PAGO EXITOSO!", Toast.LENGTH_LONG).show();
    ...
    private void doThePayment() {
        ...
        if (rdPenAccount.isChecked()) {
            double new_my_acount = basic_account_pen_db-total_amount_to_pay;
            ...
            userRef.child(currentUserID).child("credit_line_pen_used").setValue(new_credit_line_used_st);
            userRef.child(currentUserID).child("Company Line Of Credit Bills").child(postKey).child("quote_amount").setValue("0.00");
            userRef.child(currentUserID).child("Company Line Of Credit Bills").child(postKey).child("capital_amount").setValue("0.00");
            userRef.child(currentUserID).child("Company Line Of Credit Bills").child(postKey).child("monthly_interest").setValue("0.00");
            userRef.child(currentUserID).child("Company Line Of Credit Bills").child(postKey).child("debt_state").setValue("1");
        }
        if (rdUsdAccount.isChecked()) {
            double new_my_acount = basic_account_usd_db-usd_amount;
            double new_credit_line_used = credit_line_used_pen_double-capital_amount_db;
