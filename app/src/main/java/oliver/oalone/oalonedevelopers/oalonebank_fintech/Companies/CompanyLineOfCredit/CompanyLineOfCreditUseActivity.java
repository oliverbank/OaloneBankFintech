...
public class CompanyLineOfCreditUseActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_company_line_of_credit_use);
                                ...
                                oliverBankRef.addValueEventListener(new ValueEventListener() {
                                    ...
                                    public void onDataChange(DataSnapshot dataSnapshot) {
                                        if (dataSnapshot.exists()) {
                                            ...
                                            issued_capital_amount = dataSnapshot.child("Company Line Of Credit Issued Capital Amount PEN").getValue().toString();
                                            //Total Issued Capital Accumulated Amount (Without Interest) (This will be summed and then rested when user pay the debt)
                                            current_capital_amount = dataSnapshot.child("Company Line Of Credit Current Capital Amount PEN").getValue().toString();
                                            //Total Future Value Expected to Return (This will be only summed)
                                            total_future_value_amount = dataSnapshot.child("Company Line Of Credit Total Future Value Amount PEN").getValue().toString();
                                            //Current Future Value Expected to Return (This will be summed and then rested when the user pay the debt)
                                            current_future_value_amount = dataSnapshot.child("Company Line Of Credit Current Future Value Amount PEN").getValue().toString();
                                            //Total Interest Amount Accumulated (This will be only summed)
                                            total_interest_amount_accumulated = dataSnapshot.child("Company Line Of Credit Total Interest Amount PEN").getValue().toString();
                                            //Current Interest Amount (This will be summed and then rested when the user pay the debt)
                                            current_interest_amount = dataSnapshot.child("Company Line Of Credit Current Interest Amount PEN").getValue().toString();
                                            //Total Rates Accumulated in %
                                            rate_accumulated = dataSnapshot.child("Company Line Of Credit Rate Accumulated PEN").getValue().toString();
                                            //Total number of transactions
                                            transactions_number = dataSnapshot.child("Company Line Of Credit Transactions Number PEN").getValue().toString();
                                            //Number of quotes accumulated
                                            number_of_quotes = dataSnapshot.child("Company Line Of Credit Number of Quotes PEN").getValue().toString();
        ...
        spinnerQuotes = new SpinnerDialog(CompanyLineOfCreditUseActivity.this,quotes,"Selecciona el número de cuotas");
    ...
    private void updateFinancialState() {
        ...
        oliverBankRef.child("Company Line Of Credit Issued Capital Amount PEN").setValue(new_issued_capital_amount_st);
        oliverBankRef.child("Company Line Of Credit Current Capital Amount PEN").setValue(new_current_capital_amount_st);
        oliverBankRef.child("Company Line Of Credit Total Future Value Amount PEN").setValue(new_total_future_value_amount_st);
        oliverBankRef.child("Company Line Of Credit Current Future Value Amount PEN").setValue(new_current_future_value_amount_st);
        oliverBankRef.child("Company Line Of Credit Total Interest Amount PEN").setValue(new_total_interest_amount_accumulated_st);
        oliverBankRef.child("Company Line Of Credit Current Interest Amount PEN").setValue(new_current_interest_amount_st);
        oliverBankRef.child("Company Line Of Credit Rate Accumulated PEN").setValue(new_rate_accumulated_st);
        oliverBankRef.child("Company Line Of Credit Transactions Number PEN").setValue(new_transactions_number_st);
        oliverBankRef.child("Company Line Of Credit Number of Quotes PEN").setValue(new_number_of_quotes_st);
    ...
    private void registerOperation() {
        ...
        myOperationsRef.child(currentUserID+operationRandomName).updateChildren(myOperationMap).addOnCompleteListener(new OnCompleteListener() {
            @Override
            public void onComplete(@NonNull Task task) {
                Intent intent = new Intent(CompanyLineOfCreditUseActivity.this, CreditLineUseCompletedActivity.class);
    ...
    private void generateBills() {
        ...
        for (int i = 1; i <= quotes_number; i++) {
            ...
            userRef.child(currentUserID).child("Company Line Of Credit Bills").addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    ...
                    if (dataSnapshot.hasChild(current_year+""+current_month)) {
                        ...
                        userRef.child(currentUserID).child("Company Line Of Credit Bills").child(current_year+""+current_month).child("quote_amount").setValue(quote_amount_st);
                        userRef.child(currentUserID).child("Company Line Of Credit Bills").child(current_year+""+current_month).child("fixed_quote").setValue(quote_amount_st);
                        userRef.child(currentUserID).child("Company Line Of Credit Bills").child(current_year+""+current_month).child("capital_amount").setValue(capital_amount_st);
                        userRef.child(currentUserID).child("Company Line Of Credit Bills").child(current_year+""+current_month).child("monthly_interest").setValue(monthly_interes_st);
                        ...
                        userRef.child(currentUserID).child("Company Line Of Credit Bills").child(current_year+""+current_month).updateChildren(hashMap).addOnCompleteListener(new OnCompleteListener() {
                            @Override
                            public void onComplete(@NonNull Task task) {
