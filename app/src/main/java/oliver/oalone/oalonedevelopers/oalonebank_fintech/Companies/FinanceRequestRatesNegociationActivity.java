...
public class FinanceRequestRatesNegociationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_finance_request_rates_negociation);

        postKey = getIntent().getExtras().get("PostKey").toString();

        userRef = FirebaseDatabase.getInstance().getReference().child("Users");
        postRef = FirebaseDatabase.getInstance().getReference().child("Finance Requests").child(postKey).child("Rate Negociations");
        ...
        clickPostRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                {
                    String ammount = dataSnapshot.child("ammount").getValue().toString();
                    String main_currency = dataSnapshot.child("main_currency").getValue().toString();
                    String financing_frecuency = dataSnapshot.child("financing_frecuency").getValue().toString();
                    String interest_rate = dataSnapshot.child("interest_rate").getValue().toString();
                    ...
                    Picasso.with(FinanceRequestRatesNegociationActivity.this).load(company_image).fit().into(companyImage);
                    txtCompanyName.setText(company_name);
                    txtInvestment.setText("Inversión: "+ammount+" "+main_currency);
                    txtPaymentFrecuency.setText("Frecuencia de pagos: "+financing_frecuency);
                    txtInterestRate.setText("Tasa de retorno inicial: "+interest_rate+"% Anual");
                }
            }
    ...
    private void displayComments() {
        FirebaseRecyclerAdapter<RatesNegociationModel, CommentViewHolder> firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<RatesNegociationModel, CommentViewHolder>
                (RatesNegociationModel.class,R.layout.user_rate_negociation_layout,CommentViewHolder.class,postRef) {
            @Override
            protected void populateViewHolder(CommentViewHolder viewHolder, RatesNegociationModel model, int position) {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                postRef.child(randomKey).updateChildren(commentMap).addOnCompleteListener(new OnCompleteListener() {
                    ...
                    public void onComplete(@NonNull Task task) {
                        ...
                        {
                            Toast.makeText(FinanceRequestRatesNegociationActivity.this, "Error", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            }
