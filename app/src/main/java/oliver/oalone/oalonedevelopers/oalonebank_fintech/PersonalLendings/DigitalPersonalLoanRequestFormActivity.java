...
public class DigitalPersonalLoanRequestFormActivity extends AppCompatActivity {

    String credit_request_currency, credit_request_condition;
    ImageView imgCondition;
    TextView txtCondition,txtCreditScore,txtRequestedAmount,txtRequestedCurrency,txtFinanceMonth,txtFinanceFrecuency,txtGracePeriod,txtQuoteAmount,txtTotalDebt,txtInterestRate;
    ...
    String ammount,interest_rate,financing_months,financing_frecuency,grace_period,monthly_interest_rate,quote_ammount,total_debt,cost_of_debt;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_digital_personal_loan_request_form);

        postImagesReference = FirebaseStorage.getInstance().getReference();
        mAuth = FirebaseAuth.getInstance();
        currentUserID = mAuth.getCurrentUser().getUid();
        userRef = FirebaseDatabase.getInstance().getReference().child("Users");
        creditLineRequestRef = FirebaseDatabase.getInstance().getReference().child("Company Personal Lending Request");
        ...
        txtRequestedAmount = findViewById(R.id.txtRequestedAmount);
        txtRequestedCurrency = findViewById(R.id.txtRequestedCurrency);
        txtFinanceMonth = findViewById(R.id.txtFinanceMonth);
        txtFinanceFrecuency = findViewById(R.id.txtFinanceFrecuency);
        ...
        interest_rate = getIntent().getStringExtra("interest_rate");
        financing_months = getIntent().getStringExtra("financing_months");
        financing_frecuency = getIntent().getStringExtra("financing_frecuency");
        grace_period = getIntent().getStringExtra("grace_period");
        monthly_interest_rate = getIntent().getStringExtra("monthly_interest_rate");
        quote_ammount = getIntent().getStringExtra("quote_ammount");
        total_debt = getIntent().getStringExtra("total_debt");
        cost_of_debt = getIntent().getStringExtra("cost_of_debt");

        double amount_db = Double.parseDouble(ammount);
        String amount_st = decimalFormat.format(amount_db);
        if (credit_request_currency.equals("PEN")) {
            txtRequestedCurrency.setText(" Moneda del préstamo: (S/) Soles peruanos");
            txtRequestedAmount.setText(" Monto solicitado: S/ "+amount_st+" "+credit_request_currency);
        }
        if (credit_request_currency.equals("USD")) {
            txtRequestedCurrency.setText(" Moneda del préstamo: ($) Dólares americanos");
            txtRequestedAmount.setText(" Monto solicitado: $ "+amount_st+" "+credit_request_currency);
        }

        if (interest_rate.equals("0.00")) {
            txtInterestRate.setText(" Tasa de interés efectiva anual: No especificado");
        } else {
            txtInterestRate.setText(" Tasa de interés efectiva anual: "+interest_rate+"%");
        }

        if (financing_months.equals("0.00")) {
            txtFinanceMonth.setText(" Meses que durará el préstamo: No especificado");
        } else {
            txtFinanceMonth.setText(" Meses que durará el préstamo: "+financing_months+" meses");
        }

        if (financing_frecuency.equals("0.00")) {
            txtFinanceFrecuency.setText(" Frecuencia de Pagos: No especificado");
        } else {
            txtFinanceFrecuency.setText(" Frecuencia de Pagos: "+financing_frecuency);
        }

        if (grace_period.equals("0.00")) {
            txtGracePeriod.setText(" Período de Gracia: No especificado");
        } else {
            txtGracePeriod.setText("  Período de Gracia: "+grace_period+" meses");
        }

        if (quote_ammount.equals("0.00")) {
            txtQuoteAmount.setText(" Monto de la cuota: No especificado");
        } else {
            txtQuoteAmount.setText(" Monto de la cuota: "+quote_ammount+" "+credit_request_currency);
        }

        if (total_debt.equals("0.00")) {
            txtTotalDebt.setText(" Deuda total: No especificado");
        } else {
            txtTotalDebt.setText("  Deuda total: "+total_debt+" "+credit_request_currency);
        }

        if (cost_of_debt.equals("0.00")) {
            txtInterestRate.setText(" Interéses: No especificado");
        } else {
            txtInterestRate.setText(" Interéses: "+cost_of_debt+" "+credit_request_currency);
        }
        ...
        if (credit_request_currency.equals("PEN"))
        {
            txtCreditScore.setText("Préstamo personal digital en: Soles (S/) - PEN");
        }
        if (credit_request_currency.equals("USD"))
        {
            txtCreditScore.setText("Préstamo personal digital en: Dólares ($) - USD");
        }
        ...
        spinnerDegree = new SpinnerDialog(DigitalPersonalLoanRequestFormActivity.this,degree, "Selecciona tu carrera");
        ...
        spinnerSemesterNumber = new SpinnerDialog(DigitalPersonalLoanRequestFormActivity.this,semester_number, "Selecciona el semestre");
        ...
        spinnerSchools = new SpinnerDialog(DigitalPersonalLoanRequestFormActivity.this,schools,"Selecciona tu Institución Educativa");
        ...
        filePath.putFile(imageUri).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
            ...
            public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                ...
                else
                {
                    ...
                    Toast.makeText(DigitalPersonalLoanRequestFormActivity.this, "Error: "+message, Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void savingPostInformationToDatabase() {
        ...
        userMap.put("ammount",ammount);
        userMap.put("interest_rate",interest_rate);
        userMap.put("financing_months",financing_months);
        userMap.put("financing_frecuency",financing_frecuency);
        userMap.put("grace_period",grace_period);
        userMap.put("quote_ammount",quote_ammount);
        userMap.put("total_debt",total_debt);
        userMap.put("cost_of_debt",cost_of_debt);
    ...
    private void changeCreditLineRequestState() {
        ...
        userRef.child(currentUserID).updateChildren(userMap).addOnCompleteListener(new OnCompleteListener() {
            ...
            public void onComplete(@NonNull Task task) {
                loadingBar.dismiss();
                Intent intent = new Intent(DigitalPersonalLoanRequestFormActivity.this, MyAccountActivity.class);
