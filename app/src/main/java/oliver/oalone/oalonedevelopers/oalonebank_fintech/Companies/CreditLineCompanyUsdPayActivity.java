...
public class CreditLineCompanyUsdPayActivity extends AppCompatActivity {
    ...
    double ammount, last_debt,my_pen_account_double,money_account_pen,credit_line_total_pen_double,credit_line__used_pen_double,credit_line_available_pen_double,credit_line_to_chage_now_db;
    ...
    double credit_month_double1,credit_month_double2,credit_month_double3,credit_month_double4,credit_month_double5,credit_month_double6,credit_month_double7,
            ...
            new_payment, quotes_acomulated, current_user_debt;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_credit_line_company_usd_pay);
        ...
        userRef.child(currentUserID).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                money_account_pen = Double.parseDouble(basic_account_usd);
                ...
                oliverBankRef.addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            credit_line_to_chage_now = dataSnapshot.child("Company Credit Line Amount to Charge Now USD").getValue().toString();
        ...
        rdUsdAccount.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                else
                {
                    ...
                    txtCreditLinePaymentMonthPen.setText("PAGO DEL MES: $ "+current_debt_st);
                    txtMonth.setText("Mes a pagar: "+month);
                    txtxDebt.setText("Deuda actual: $"+current_debt_st);
                    txtRealAmmount.setText("Monto a pagar: $"+ammount_st);

                    last_debt = current_user_debt-ammount;
                    String last_debt_st = decimalFormat.format(last_debt);
                    if (last_debt <= 0.00)
                    {
                        txtLastDebt.setText("Deuda restante: $0.00");
                    }
                    if (last_debt > 0.00)
                    {
                        txtLastDebt.setText("Deuda restante: $"+last_debt_st);
                    }
    ...
    private void paymentCreditLine() {
        ...
        if (month == 1)
        {
            hashMap.put("credit_line_usd_payment_month1",last_debt_st);
        }
        if (month == 2)
        {
            hashMap.put("credit_line_usd_payment_month2",last_debt_st);
        }
        if (month == 3)
        {
            hashMap.put("credit_line_usd_payment_month3",last_debt_st);
        }
        if (month == 4)
        {
        ...
        if (month == 5)
        {
            hashMap.put("credit_line_usd_payment_month5",last_debt_st);
        }
        if (month == 6)
        {
            hashMap.put("credit_line_usd_payment_month6",last_debt_st);
        }
        if (month == 7)
        {
            hashMap.put("credit_line_usd_payment_month7",last_debt_st);
        }
        if (month == 8)
        {
            hashMap.put("credit_line_usd_payment_month8",last_debt_st);
        }
        if (month == 9)
        {
            hashMap.put("credit_line_usd_payment_month9",last_debt_st);
        }
        if (month == 10)
        {
            hashMap.put("credit_line_usd_payment_month10",last_debt_st);
        }
        if (month == 11)
        {
            hashMap.put("credit_line_usd_payment_month11",last_debt_st);
        }
        if (month == 12)
        {
            hashMap.put("credit_line_usd_payment_month12",last_debt_st);
        }

        ammount = Double.parseDouble(edtAmmount.getText().toString());
        double ammount_in_account = money_account_pen-ammount;
        String ammount_in_account_st = decimalFormat.format(ammount_in_account);
        hashMap.put("basic_account_usd",ammount_in_account_st);

        //Update credit line
        double new_used_credit_line = credit_line__used_pen_double-ammount;
        String new_used_credit_line_st = decimalFormat.format(new_used_credit_line);
        hashMap.put("credit_line_usd_used",new_used_credit_line_st);
        ...
        userRef.child(currentUserID).updateChildren(hashMap).addOnCompleteListener(new OnCompleteListener() {
            ...
            public void onComplete(@NonNull Task task) {
                if (task.isSuccessful())
                {
                    ...
                    oliverBankRef.child("Company Credit Line Amount to Charge Now USD").setValue(real_amount_tocharge_st);
                    registerOperation();
                }
            }
        });
    ...
    private void registerOperation() {
        ...
        //Credit Line Payment
        myOperationMap.put("credit_payment_ammount",edtAmmount.getText().toString()+" USD");
        ...
        myOperationsRef.child(currentUserID+operationRandomName).updateChildren(myOperationMap).addOnCompleteListener(new OnCompleteListener() {
            @Override
            public void onComplete(@NonNull Task task) {
                Intent intent = new Intent(CreditLineCompanyUsdPayActivity.this, InvesmentFundTransactionCompletedActivity.class);
                intent.putExtra("PostKey", postKey);
                intent.putExtra("TransactionCode",operationRandomName+"CCL");
    ...
    private void showAccoutDialog() {
        AlertDialog.Builder dialog = new AlertDialog.Builder(this);
        dialog.setTitle("Debes usar una cuenta en dólares");
        dialog.setIcon(R.drawable.money2_icon);
        dialog.setMessage("Para cumplir con tus obliaciones en dólares debes convertir tu dinero a dólares.");
